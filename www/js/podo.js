...
function Kalman() {
	this.G  = 1; // Gt/Kt - filter gain
	this.Rw = 1; // Rt - noise power desirable  (the covariance of the noise mesured)
	this.Rv = 10; // Rt - noise power estimated (the covariance of the noise mesured)
	
	this.A = 1; // at - signal estimated at time 
	this.C = 1; // Ht - matrix of the estimation model
	this.B = 0;
	this.u = 0;
	this.P = NaN; // Pt|t-1 - error covariance matrix
	...
	this.onFilteringKalman = function(ech)//signal: signal measured
	{
		...
		if (isNaN(this.x)) {
			...
			// Gain
			this.G = this.P*this.C*1/(this.C*this.P*this.C+this.Rv); //Gt = Pt|t-1 * Ht^T(Ht * Pt|t-1 * Ht^T + Rt)^-1  (Kt)
			// Correction
			this.x = this.x + this.G*(this.y-this.C*this.x);
			this.P = this.P - this.G*this.C*this.P; //Pt|t = Pt|t-1 - Kt * Ht * Pt|t-1
		};
		return this.x;
	};	
